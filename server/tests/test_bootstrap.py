...
class TestCase(unittest.TestCase):
    ...
    def test_load_sample_data_from_excel(
        self,
        m_application_objects,
        m_shortcut_objects,
        m_usershortcut_objects,
        m_user_objects,
    ):
        ...
        m_user_objects.get.return_value = django.contrib.auth.models.User(
            username="test"
        )
        ...
        assert len(m_application_objects.bulk_create.call_args[0][0]) == len(
            df_applications
        )
        ...
        df_user_shortcuts["application_command"] = (
            df_user_shortcuts["application_name"] + df_user_shortcuts["command"]
        )
        assert len(m_shortcut_objects.bulk_create.call_args[0][0]) == len(
            df_user_shortcuts["application_command"].unique()
        )

        # Ensure all UserShortcut objects are created
        assert len(m_usershortcut_objects.bulk_create.call_args[0][0]) == len(
            df_user_shortcuts
        )
